...
- `api/aiwsim_api/` FastAPI service (contract §3, §9, §13, §15–16): `service.py` (run cache, compare, explain shared by HTTP and chat), `chat.py` (Claude tool loop; tools only read results and run scenarios), `insights.py` (deterministic ranked findings), `brief.py` (Markdown/HTML briefs), `levers.py`, `export_static.py` (serverless demo bundle, §18). `web/` Vue 3 + TypeScript + Pinia + D3 (no React); `VITE_USE_MOCK=1` synthetic data, `VITE_STATIC=1` precomputed bundle.
