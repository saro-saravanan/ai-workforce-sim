...
- The baseline's `labor.layoff_first_share` (0.25) is fitted to announced AI-cited job cuts (Challenger); the spec's attrition-first rule is the φ = 0 case. Keep the Challenger scoreboard rows within a factor of two of the claims when you touch the labour block (`test_employment_levels_and_layoff_reality_rows`).
...
- Phase 9 robustness (review `docs/adversarial-review-phase8.md`, status in `docs/findings-phase9.md`): the closure is an ensemble axis (`sampling.ENSEMBLE_AXES["closure"]`, 64 cells, baseline 384 draws); the Story's futures are the closure medians, its range label is "the range of the model's assumptions", and `results.backtest` scores 2024Q1–2026Q2 against `data/processed/series/backtest.csv` (rows with `used_in_fit` are calibration targets, not tests). Diagnostics: `aiwsim convergence`, `aiwsim regional`, `aiwsim diag threshold-seeds|classifier-sample`. Any new calibration target goes on the scoreboard with `role: target`. Read `docs/current-model.md` before quoting a number.
- External data (Phase 9b): `data/external/` is written by `.github/workflows/external-data.yml` (BLS OEWS, BEA use table when reachable); `aiwsim data build` reads it through `aiwsim.data.external` and falls back to fixtures without it. Channel rules are v3 (`classify.py`); the audit labels live in `docs/classifier-audit-labels.csv` and `aiwsim diag audit` scores the rules against them. `sectors.csv` carries two labour shares: of gross output (prices, demand) and of value added (the reported productivity gain); do not use one for the other.
