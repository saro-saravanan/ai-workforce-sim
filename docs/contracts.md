...
The chat panel is a third mode of the right-hand panel (`Explain · Ask`). It sends the current `scenario_hash`, `compare_hash`, `region`, and `quarter` as context. A proposed scenario renders as a diff card with **Run** (runs through the results store, sets it as the current or compare scenario) and **Edit** (opens the levers drawer pre-filled). The `Export` menu on the top bar opens the technical brief as a page or downloads it as Markdown for the current run (with the compare run when one is selected) and links to the Story view for the executive brief. The `Ask` tab is rendered only when `GET /api/chat/status` reports `available: true`; otherwise the panel is `Explain` alone. In mock mode the panel answers with canned replies and the deterministic insights of the mock document.
...
Static mode in the client: scenarios from the manifest; `runScenario(id or hash)` from `runs/`; `compareRuns` from `compare/` when present, else the client-side paired difference; `fetchInsights` from `insights/`; brief links to `briefs/`; explain client-side from the document (as in mock mode); `fetchChatStatus` → `{available: false, reason: "static demo"}`; running a new scenario throws a friendly error and the levers drawer says so. The top bar badge reads `static demo · precomputed runs` and links to the About view's static-demo section, which says what works without a server and what needs the local API. `VITE_BASE` sets the Vite `base` for hosting under a path (GitHub Pages: `/ai-workforce-sim/`).
...
- **Seba 2026 variant** (`scenarios/preset-seba-2026.json`, parent `preset-seba-rethinkx`): RethinkX's 2024–2026 humanoid-labour claims and the 2026 roadmap as levers; new lever `levers.applications.embodiment.manipulation_automatable_share` (0.3–0.85, P.101). Both Seba presets are story futures (`service.FUTURE_SCENARIOS`) and static-export runs.
- **Forecast scoreboard** (`forecasts.csv` → results `forecasts[]`): `{source, short, region, year, metric, proxy, preset_id, claimed, unit, note, source_tag, quarter, model_central, model_p10, model_p90, verdict}` with `verdict` ∈ {`within band`, `model lower`, `model higher`} from the run's p10–p90 band at the claim's quarter. `metric` ∈ {gdp_pct, tfp_pct, embodied_displacement_share, autonomous_share_of_ride_hail (robotaxi coverage), ride_hail_driver_displacement, exposed_share, young_exposed_employment_pct, humanoid_cost_per_hour_usd (mobile-manipulation cost per worker-hour equivalent)}. Every row carries a `source_tag` saying how it was verified. `proxy = 1` marks a claim compared with the nearest model quantity rather than the same quantity.
- **Who pays for AI** (`series[region].ai_spend_by_source_bn` {automation, augmentation, content, total} and `ai_spend_by_occupation_group_bn` {group title: percentiles} for the largest eight paying groups plus `Other groups`): employers replacing tasks with software, employers buying tools that speed up workers, and consumers paying for AI-made content; the occupation-group split says whose work the software spend buys. `ai_rents_received_bn` (by value-chain stage) is the split of this spending across all regions, so a region's AI income is explained by the world's sources. The story's money beat carries `extra_chart` (a bars chart of the world sources and the top paying groups) and `income` {sources_world_bn, paying_groups_world_bn, received_by_stage_bn}; beats may carry `extra_chart` in general and the web renders it under the main chart.
- **Levels and the layoffs reality check.** `series[region].baseline_employment_level` (the frozen-AI path in heads, population and normal growth) and `employment_level` (the same quantity with AI, identical to the headline by construction) let the jobs beat say how 2040 compares with today; the beat carries `levels` {today, without_ai, with_ai: {p10, p50, p90}} and an `extra_chart`. Lever `levers.labor.layoff_first_share` (0–1; baseline 0.25, fitted to Challenger's AI-cited job-cut counts) is the share of each required cut taken as layoffs before attrition. Scoreboard rows may carry `quarter` (else `{year}Q4`); metrics `ai_layoffs_in_year` and `ai_layoffs_cum` score the Challenger rows. The hiring beat carries `reality_check` (the Challenger rows) and reads `variant_docs` (service `VARIANT_SCENARIOS`, the layoffs-first run; export manifest `variant_scenarios`).
- **Investment versus returns.** Results carry `investment` {years, rows: [{year, capex_model_bn, capex_observed_bn, producer_revenue_bn, ai_spend_bn, gdp_gain_bn, productivity_gain_bn, investment_in_gdp_bn}], cumulative_2024_to_horizon, gdp_2024_bn, notes} (world, central run, annual at Q4). `capex_model_bn` is the hyperscaler capex path the model assumes (P.80–P.82); `capex_observed_bn` sums the four hyperscalers from `data/processed/series/capex.csv` (calendar basis preferred); `producer_revenue_bn` is AI producers' value-chain revenue over all regions, equal to what employers and consumers spend on AI in the model; `gdp_gain_bn` is the GDP effect at 2024 GDP, `productivity_gain_bn` its TFP part. Scoreboard metrics `hyperscaler_capex_bn` and `ai_producer_revenue_bn` score the observed capex and the frontier labs' revenue run rate. The story carries `investment` {paragraphs, rows, cumulative, payback_year_revenue, payback_year_productivity, chart, definition}; the web renders it as a section between the policy runs and the scoreboard, and the executive brief as "Investment versus returns".
- **Revenue layer** (spec §A.16): `series[region].ai_spend_bn` is now employers' spend at market prices plus consumer AI spending paid from the region; `ai_spend_at_token_cost_bn` is the cost basis; `ai_consumer_revenue_bn` the consumer part; `ai_spend_by_source_bn` gains `consumer`. `meta.price_multiple_path` and `meta.consumer_ai_revenue_path_bn` carry the two paths (central). Investment rows gain `consumer_revenue_bn` and `spend_at_cost_bn`. Scoreboard rows may carry `claimed_low`/`claimed_high`; the verdict is "within band" when the model's 10–90 band meets the claim's range.
- **Scoreboard labels** (`short`) are unique per row (a source with two claims gets two labels, e.g. "Seba 2017, passenger miles" and "Seba 2017, drivers (implied)").
- **Static export** adds the policy scenarios and the Seba preset to the default run list, `story/<id>.json` (§26, the U.S. story: policies against the exported baseline, the Seba preset as a future), `story/<id>.<region>.json` for every other region of the run (manifest key `story_regions`: scenario id → region id → file; the U.S. entry points at `story/<id>.json`), `briefs/<id>.exec.md|html` (U.S.; the client builds the other regions' executive brief from their story), and manifest keys `story`, `story_regions`, `exec_briefs`, `policy_scenarios`, `future_scenarios`. `region=WORLD` (API and export, `story/<id>.WORLD.json`) is the server-side aggregate of the ten regions under the web app's World rules (rates and shares employment-weighted per percentile, counts and dollars summed, the capability clock the frontier), with its own jobs ledger. The story document carries `region_name` beside `region`; outside the U.S. the hiring beat reads the region's own series, the cohort beat is marked as U.S. detail, and the waves beat ranks occupations by the region's automated task share instead of U.S. employment effects.

# Phase 9 additions (contracts v0.10): robustness after the adversarial review

## 29. Backtest, closure axis, configuration, diagnostics

- **Backtest** (`data/processed/series/backtest.csv` → results `backtest` {horizon, rows, summary, notes}): observed series 2024Q1–2026Q2 with the model's central value at each quarter and the error; `model_metric` ∈ {adoption_share_firm_weighted, ai_layoffs_cum, ai_producer_revenue_bn, hyperscaler_capex_bn, none}; `used_in_fit` marks rows that set a parameter. The story carries `backtest` with plain sentences; the executive brief a section "How the model has done so far".
- **Macro closure**: sixth structural-ensemble axis `closure ∈ {demand, no_demand_feedback}` (64 cells; baseline draws 384); lever `levers.macro.closure` forces one closure. The story's named futures are the cell medians under each closure; `structural_spread` {min, max, cells, agree_on_sign} is the range of cell medians and drives the jobs beat's sureness (a coin flip when cells disagree on sign). P.87's range is 0.3–0.9.
- **Configuration**: scenarios may carry `regions: [ids]`; `config-us-closed` runs the U.S. alone. Lever `levers.applications.hardware.ramp_allocation ∈ {global, local}`.
- **Scoreboard roles**: `forecasts[].role ∈ {target, comparison}`; `claimed_low`/`claimed_high` optional; the verdict is "within band" when the model's band meets the claim's range.
- **Diagnostics**: `aiwsim convergence` (p10/p50/p90 by draw count and seed, `docs/convergence.md`), `aiwsim regional` (U.S. headline under the ten-region, U.S.-only, local-ramp and no-traded-services configurations, `docs/regional-decomposition.md`), `aiwsim diag threshold-seeds` and `aiwsim diag classifier-sample` (`docs/diagnostics-phase9.md`, `docs/classifier-audit-sample.md`).
- **Phase 9 engine levers** (from the robustness items): `levers.applications.hardware.cost_floor_scale` (0–3; scales `embodiment_classes.csv.cost_floor_usd_per_hour`, which floors the hardware cost per worker-hour in the cost test and the recorded series), `levers.capability.threshold_seed` (0–999; re-derives the per-task threshold hash, 0 is the reference), P.146/P.147 (entrant supply elasticity and lag: the share of an attrition cut that lands on the entrant cohort is `min(1, (w/w0)^ε)` at the lagged relative wage). Policy financing: the income-tax surcharge is booked as revenue (`fiscal = tax + surcharge − cost`), a universal payment is spent at the P.86 average propensity; scenarios `policy-ubi-deficit` (deficit-financed, validity flag) and `variant-market-clearing-wages` (P.73, P.74, P.61 and the P.60 scale at the top of their ranges) ship. `service.VARIANT_SCENARIOS` carries both variants; the story reads the market-clearing variant into the pay beat.
- **Labels**: "Range of the model's assumptions" replaces "Likely range" everywhere a reader sees it; the first caveat says the model is a structured scenario model.

## 30. Phase 9b: sector tables, input-output propagation, hold-out, exposure source, audit, fleet rows

- **External data** (`data/external/`, fetched by `.github/workflows/external-data.yml` on a GitHub runner because bls.gov and bea.gov are unreachable from the build environment): BLS OEWS May 2025 `natsector_M2025_dl.xlsx`, `national_M2025_dl.xlsx`, `state_M2025_dl.xlsx`; BEA summary use table named by `data/external/bea/USE_TABLE_FILE` when fetched (the workflow tries live names, the Internet Archive index and the BEA API with the `BEA_API_KEY` secret). `aiwsim data build` reads them through `aiwsim.data.external`: `occupations.csv` employment and wages refreshed to the OEWS vintage, `occ_sector.csv` (830 occupations × 20 NAICS sectors, real), `occ_state.csv` and `states.csv` (real), `sectors.csv` (20 sectors: tradable from the fixture list, `labor_cost_share` and `consumption_share` from BEA when present else authors' estimates `external.SECTOR_E`, `demand_elasticity` E, `friction` 1.0), `io_direct_requirements.csv` (20 × 20 `a_ij`, BEA only). Without the external files the Phase 1 fixtures are built, so a clone still builds.
- **Engine**: `Inputs.labor_cost_share_mean` (employment-weighted) replaces the unweighted mean in the output-substitution block; lever `levers.macro.io_propagation ∈ {on, off}` applies `dlnP = (I − Aᵀ)⁻¹ dlnc_direct` per region when the direct-requirements table exists; lever `levers.capability.exposure_source ∈ {gpts, aioe}` scales each occupation's ever-automatable mass by the ratio of its AIOE exposure (rank-mapped onto the GPTs-are-GPTs beta scale, `data/processed/exposure_aioe.csv`, built from the fetched appendix and never committed: the AIOE repository carries no license) to its GPTs beta.
- **Channel rules v3** (`classify.CHANNEL_VERSION`): the driving transport rule needs a vehicle cue and no manual-handling cue; "collect … customers" and noun forms of delivery no longer drive; outside the production groups (SOC 51, 53-7, 45) a physical statement reaches the manipulation channel only with a handling verb on a physical object, otherwise `none`; the fixed rules' generic equipment terms are gated to the production groups; agricultural spraying is aerial. `aiwsim diag audit` scores the rules against `docs/classifier-audit-labels.csv` (`docs/classifier-audit-agreement.md`).
- **Backtest**: series `robotaxi_fleet_us` (model metric `driving_fleet_units_us`, the deployed driving-class units in the U.S.).
- **Diagnostics**: `aiwsim diag holdout` (refit P.143 and the layoff share to 2025 rows only, score the 2026 rows; `docs/holdout-2026.md`), `aiwsim diag exposure-source` (headline and occupation ranking under the two exposure sources).
