...
- **Who pays for AI** (`series[region].ai_spend_by_source_bn` {automation, augmentation, content, total} and `ai_spend_by_occupation_group_bn` [{major_group, title, spend_bn}] for the largest eight paying groups plus `other`): employers replacing tasks with software, employers buying tools that speed up workers, and consumers paying for AI-made content; the occupation-group split says whose work the software spend buys. `ai_rents_received_bn` (by value-chain stage) is the split of this spending across all regions, so a region's AI income is explained by the world's sources. The story's money beat carries `extra_chart` (a bars chart of the world sources and the top paying groups) and `income` {sources_world_bn, paying_groups_world_bn, received_by_stage_bn}; beats may carry `extra_chart` in general and the web renders it under the main chart.
