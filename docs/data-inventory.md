...
- **BEA input-output summary use table 2024** (`data/external/bea/use_summary_2024_api.json`, BEA API, InputOutput dataset table 259, fetched by the workflow with the `BEA_API_KEY` secret in the `ai-workforce-sim` environment; public domain). Built into `sectors.csv` (labour's share of gross output and of value added, consumption shares by producing sector) and `io_direct_requirements.csv` (20 × 20). Parsers for the spreadsheet and the API JSON are in `sim/aiwsim/data/ingest/bea_io.py`; BEA summary codes map to the 20 NAICS sectors by their leading digits with the named exceptions (HS/ORE → 53, G* → 92, 4A0 → 44-45).
