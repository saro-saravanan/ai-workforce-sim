...
`aiwsim diag threshold-seeds --seeds 0,1,2 --regions US` on the baseline scenario, central run (draw 0), no tornado or channels (rerun after Phase 9b's data refresh; the Phase 9 values were −2.54 / −7.31). Seed 0 is the reference hash of the task key; seeds 1 and 2 append the seed to the key before hashing, which re-spreads every task group's threshold within its occupation's range. Occupation effects are 2040Q4 employment versus the frozen-AI path; the rank correlation and the top-decile overlap compare each seed's ranking of the 831 occupations with seed 0.
...
|---|---|---|---|---|---|---|---|
| 0 | -3.58 | -8.85 | +0.00 | +4.71 | 1.0000 | 1.00 | 0.00 |
| 1 | -3.59 | -8.85 | +0.00 | +4.71 | 0.9999 | 0.99 | 0.85 |
| 2 | -3.58 | -8.85 | +0.00 | +4.71 | 0.9999 | 1.00 | 3.85 |
