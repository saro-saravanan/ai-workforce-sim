...
The preset is labelled as levers, and the forecast rows are transcribed from recollection (`source_tag` V?) until the reports are fetched and cited (`docs/data-inventory.md`).
...
- Three of seven scoreboard rows compare a claim with the nearest model quantity (`proxy = 1`), and their verdicts are about direction and size.
