...
| Channel rules v3 (§A.2; Phase 9b audit) | keyword rules v2 | 120-statement audit labelled by the model as reviewer: v2 agreed with the labels on 57% of statements (manipulation precision 25%, driving 38%); v3 requires a vehicle cue for transport statements, a handling verb on a physical object for manipulation outside the production groups, and gates generic equipment terms to production groups: agreement 74%, manipulation task-hours 18% → 12% of all task-hours, the rest to `none` | "collect payments", "transport records" and "conduct orientation" were robot targets by keyword |
| Sector layer (§1.2, §6.2; Phase 9b, review item 10) | one sector (fixture) | 20 NAICS sectors with the OEWS May 2025 occupation × sector matrix; sector labour-cost shares and consumption shares from the BEA use table when fetched, authors' estimates until then; input-output cost propagation `dlnP = (I − Aᵀ)⁻¹ dlnc` (lever `macro.io_propagation`) once the BEA direct-requirements matrix is present | the model could make no sectoral claim; the single-sector fixture also fixed every sector's labour share at 0.58 |
| Exposure source (§2.2; Phase 9b, review §2.4) | GPTs-are-GPTs task labels only | lever `capability.exposure_source = aioe` rescales each occupation's ever-automatable mass by its AIOE rank-mapped exposure; `aiwsim diag exposure-source` reports the headline and the occupation ranking under both | a reviewer asked whether the ranking of occupations survives a different exposure source |
