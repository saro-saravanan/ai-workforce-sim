...
| Regional access (§3.3) | `C_{r,t} = max over available actors of (C_t − λ_a − δ^reg)` | each region reads the shared task layer at an **access lag** in quarters: `max(δ^reg, domestic frontier lag)`, adjusted by the EU AI Act and export-control levers; the lag is reported in `meta.regions[].access_lag_quarters` | one task layer per quarter instead of one per region; exact when the clock is locally linear |
| Regional profitability (§2.4) | task-level test at regional wages | three wage tiers for the profitability test (1.0, 0.25, 0.10 of U.S. wages); integration cost scales with the tier; regions above half the U.S. wage share the U.S. tier | κ is far below wages except in low-wage regions, so tiers only matter there |
| Regional occupation structure (§1.1) | ISCO employment by country through the crosswalk | FIXTURE: U.S. occupational mix tilted by log GDP per capita, scaled to regional employment; wages = U.S. wage × regional wage level; U.S. cohort structure applied to every region | ILOSTAT/Eurostat unreachable offline; `ingest/ilostat.py`, `ingest/eurostat_lfs.py` replace it |
| Market shares (§3.6) | softmax over capability gap and price | deterministic per region from actor frontier lag, list price relative to the median, and availability (export-control lever removes U.S. labs from China); constant over time in v0.2 | no draw dimension on market shares; rents still vary with spend |
| Rents (§6.3) | four stages by value chain | model → labs' home regions by market share; compute → data-center location (EU localization lever); chips → fixed US/TW/EU split; integration → domestic. **Net AI trade** (rents received − spend) enters regional GDP; the hardware half of incremental capex is booked as exports of the chip regions and the other half as domestic investment where the data centers are | conserves spend globally (tested) |
| Trade linkage (§6.3) | tradable demand includes other regions' consumption via import shares | implemented with a sales matrix from `trade_weights` × destination GDP; inert while the sector fixture marks everything non-tradable | activates with the OEWS industry ingest |
| Fitted parameters in the Monte Carlo (§7.1, §7.4) | draws around registry centrals | the BTOS-fitted Bass `q` (P.42) replaces the registry central before drawing, so draw 0 and a single central run coincide (tested); an explicit override of P.42 wins | Phase 5 found the MC central line using the registry value (adoption 65% in 2030) while single runs used the fitted one (77%) |
| Spillover (§4.2) | `q^×` over all regions | spillover from the U.S. adoption ratio only, with `spillover_weight_us` per region and lag `P.44` | the U.S. leads adoption in every scenario the tool ships |
