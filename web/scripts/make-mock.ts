 ...
 * eight-entry channels, `hours_cut_self`). Run: pnpm make-mock
 *
...
// ---------- shared fixtures ----------
/** contracts §20 order: embodied and adjacent are the Phase 6 channels */
...
/** Mechanism cells (spec §7.2, v0.3 §A.7): demand × reinstatement × pass-through × hardware learning rate. */
...
for (const a of CELL_AXES[0]!)
  for (const b of CELL_AXES[1]!)
    for (const c of CELL_AXES[2]!) for (const d of CELL_AXES[3]!) cells.push(`${a}|${b}|${c}|${d}`)
...
function build(cfg: Cfg): ResultsDocument {
  ...
  const axisEffect: Record<HeadlineMetric, [number, number, number, number]> = {
    employment_pct_vs_baseline: [0.22, -0.35, 0.1, -0.08],
    gdp_pct_vs_baseline: [0.12, 0.08, 0.18, 0.05],
    real_wage_pct_vs_baseline: [0.1, 0.25, 0.6, 0.04],
    wage_share_pp_vs_baseline: [0.05, 0.1, -0.3, -0.03],
  }
  ...
  for (const m of HEADLINE_METRICS) {
    ...
    cells.forEach((id, ci) => {
      const bits = [(ci >> 3) & 1, (ci >> 2) & 1, (ci >> 1) & 1, ci & 1]
      const eff = axisEffect[m]
      let f = 1
      for (let a = 0; a < 4; a++) if (bits[a]) f *= 1 + (eff[a] ?? 0)
  ...
  return {
    ...
    channels: {
      employment_pct_vs_baseline: {
        order: channelOrder,
        contributions: rescale(empRaw, ['automation', 'embodied'], S.employment.p50),
      },
      gdp_pct_vs_baseline: {
        order: channelOrder,
...
console.log(
  `wrote ${quarters.length} quarters, ${docA.occupations.length} occupations, ${docA.states.length} states, ${REGIONS.length} regions, ${docA.world?.length} countries (${worldGeo.features.length} drawn), ${docA.supply?.releases.length} releases, ${docA.applications?.length} applications, ${docA.channels.employment_pct_vs_baseline?.order.length}-entry channels, ${levers.length} levers, ${scenarioDocs.length} scenarios → ${outDir}`,
)
