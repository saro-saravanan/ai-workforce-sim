...
import type { FlowsSection, FlowDestination } from '@/types/results'
...
const m = { top: 12, right: 250, bottom: 12, left: 250 }
...
const graph = computed(() => {
  ...
  const destTotal = FLOW_DESTINATIONS.reduce(
  ...
  const nodes: NodeExtra[] = [
    ...
    ...FLOW_DESTINATIONS.map((d: FlowDestination) => ({
      id: `d:${d}`,
  ...
  for (const o of origins) {
    ...
    for (const d of FLOW_DESTINATIONS) {
    ...
    <svg :width="width" :height="height" role="img" :aria-label="`Where displaced workers went, cumulative to ${quarterLabel}`">
      ...
      <g class="nodes">
        ...
        >
          ...
          >
            {{ n.label }}
            <tspan class="val"> {{ fmtCompact(n.p50) }}</tspan>
          </text>
        </g>
      </g>
